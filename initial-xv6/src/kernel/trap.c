...
#ifdef MLFQ
void deque(int que_num, struct proc *to_remove)
{
  struct proc *i;
  for (i = mlf_Que[que_num].Process[0]; i < to_remove; i++)
  {
  }
  i++;
  for (; i < mlf_Que[que_num].Process[NPROC - 1]; i++)
  {
    *(i - 1) = *(i);
  }
  mlf_Que[que_num].Process[NPROC - 1] = 0;
}
#endif
...
void usertrap(void)
{
  ...
  {
    if (which_dev == 2 && p->trap_alarm_status == 0)
    {
      struct trapframe *newframe = kalloc();
      memmove(newframe, p->trapframe, PGSIZE);
      p->sigalarm_trap = newframe;
      p->current_ticks--;
      if (p->current_ticks <= 0)
      {
  ...
  for (struct proc *pres_proc = proc; pres_proc < &proc[NPROC]; pres_proc++)
  {
    ...
    {
      if (pres_proc->wait_time >= WAITTIME)
      {
        pres_proc->que_on = 0;
