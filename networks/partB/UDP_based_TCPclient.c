...
int main()
{
 ...
 char temp_data[] = "This is a test data from the client to server.";

 send_message(temp_data, strlen(temp_data), udp_server, server_address);
 ...
 while (1)
 {
  ...
  socklen_t client_size = sizeof(server_address);
  int recv_size = recvfrom(udp_server, &temp_chunk, sizeof(temp_chunk), 0, (struct sockaddr *)&server_address, &client_size);
  if (recv_size < 0)
  {
   continue;
  }
  ...
  sendto(udp_server, &acknowledge, sizeof(acknowledge), 0, (struct sockaddr *)&server_address, client_size);
